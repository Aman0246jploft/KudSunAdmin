...
import { getDisputeTypes, createDisputeType, updateDisputeType, deleteDisputeType } from '../../features/slices/settingSlice';
...
const DisputeTypeManagement = () => {
  ...
  const fetchReportTypes = () => {
    dispatch(getDisputeTypes({ pageNo: currentPage, size: pageSize }))
      .unwrap()
      .catch((error) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedReportType) {
        await dispatch(updateDisputeType({ id: selectedReportType._id, ...formData })).unwrap();
        toast.success('Dispute type updated successfully');
      } else {
        await dispatch(createDisputeType(formData)).unwrap();
        toast.success('Dispute type created successfully');
      }
      handleCloseModal();
      fetchReportTypes();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this report type?')) {
      try {
        ...
        toast.success('Dispute type deleted successfully');
        fetchReportTypes();
      } catch (error) {
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Dispute Type Management</h1>
        <Button variant="primary" onClick={() => handleOpenModal()}>
          Add New Dispute Type
        </Button>
      </div>
...
export default DisputeTypeManagement; 
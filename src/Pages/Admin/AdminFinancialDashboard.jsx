...
export default function AdminFinancialDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await authAxiosClient.get(
        `/order/admin/financial-dashboard?${queryParams}`
      );
  ...
  const searchProducts = async (searchTerm) => {
    ...
    try {
      const response = await authAxiosClient.get(
        `/sellProduct/search?q=${encodeURIComponent(searchTerm)}&limit=10`
      );
      if (response.data?.status) {
  ...
  const fetchProductFinancials = async (productId) => {
    try {
      setLoading(true);
      const response = await authAxiosClient.get(
        `/order/admin/product-financial/${productId}?includeHistory=true`
      );
      if (response.data?.status) {
  ...
  const fetchMoneyFlowData = async () => {
    try {
      ...
      const response = await authAxiosClient.get(
        `/order/admin/money-flow?${queryParams}`
      );
      if (response.data?.status) {
      ...
      <div className={`p-4 lg:p-6 rounded-lg border ${colorClasses[color]}`}>
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            {subtitle && (
              <p className="text-xs sm:text-sm opacity-60 mt-1">{subtitle}</p>
            )}
          </div>
  ...
  const TabButton = ({ tabKey, label, isActive, onClick }) => (
    <button
      ...
      className={`px-3 sm:px-4 lg:px-6 py-2 sm:py-3 font-medium text-xs sm:text-sm rounded-lg transition-all ${
        isActive
    ...
    <div className="p-3 sm:p-4 lg:p-6 space-y-4 sm:">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600 mt-1">
            Complete financial analytics and money flow tracking
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 bg-gray-50 p-2 rounded-lg">
        ...
        <TabButton
          ...
          isActive={activeTab === "product-analysis"}
          onClick={setActiveTab}
        />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <StatCard
              ...
              subtitle={`${formatNumber(
                dashboardData.overview.orders.totalOrders
              )} orders`}
              icon={FaShoppingCart}
              color="blue"
            />
            ...
            <StatCard
              title="Disputes"
              value={formatNumber(
                dashboardData.overview.disputes?.reduce(
                  (sum, d) => sum + d.count,
                  0
                ) || 0
              )}
            ...
            <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
              ...
              <div className="space-y-3">
                {dashboardData.platformRevenue?.map((rev, index) => (
                  <div
                    key={index}
                    className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-3 bg-gray-50 rounded space-y-1 sm:space-y-0"
                  >
                    ...
                    <div className="text-left sm:text-right">
                      <span className="font-bold  text-sm sm:text-base">
                        {formatCurrency(rev.totalAmount)}
                      </span>
                      <span className="text-xs text-gray-500 block">
                        {formatNumber(rev.count)} transactions
                      </span>
                    </div>
                  </div>
                )) || []}
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-right p-2 sm:p-3 text-xs sm:text-sm">
                      Orders
                    </th>
                    <th className="text-right p-2 sm:p-3 text-xs sm:text-sm">
                      GMV
                  ...
                  {dashboardData.dailyTrends?.slice(-7).map((day, index) => (
                    <tr key={index} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">
                        {formatNumber(day.orderCount)}
                      </td>
                      <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">
                        {formatCurrency(day.totalGMV)}
                      </td>
                      <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">
                        {formatNumber(day.uniqueBuyerCount)}
                      </td>
                      <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">
                        {formatNumber(day.uniqueSellerCount)}
                      </td>
                    </tr>
                  )) || []}
                </tbody>
              ...
              <div className="space-y-4 sm:">
                <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-center sm:text-left">
                    <h4 className="font-bold text-lg">
                      {productFinancialData.product.title}
                    </h4>
                    <p className="text-gray-600">
                      By {productFinancialData.product.userId?.userName}
                    </p>
                    <p className="text-sm text-gray-500">
                      Status:{" "}
                      {productFinancialData.product.isActive
                        ? "Active"
                        : "Inactive"}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard
                    title="Total Sales"
                    value={formatNumber(
                      productFinancialData.financialSummary.totalSales
                    )}
                    subtitle={formatCurrency(
                      productFinancialData.financialSummary.totalRevenue
                    )}
                    icon={FaShoppingCart}
                    color="blue"
                  />
                  <StatCard
                    title="Platform Revenue"
                    value={formatCurrency(
                      productFinancialData.platformRevenue.total
                    )}
                  ...
                  <StatCard
                    title="Seller Earned"
                    value={formatCurrency(
                      productFinancialData.sellerPayouts.totalPaidToSeller
                    )}
                  ...
                  <StatCard
                    title="Disputes"
                    value={formatNumber(
                      productFinancialData.disputes?.total || 0
                    )}
                    subtitle={`${formatCurrency(
                      productFinancialData.disputes?.totalFinancialImpact || 0
                    )} impact`}
                    icon={FaExclamationTriangle}
                    color={
                      productFinancialData.disputes?.total > 0
                      ...
                      <table className="min-w-full">
                        <thead>
                          <tr className="bg-gray-50">
                            ...
                            <th className="text-center p-2 sm:p-3 text-xs sm:text-sm">
                              Status
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {productFinancialData.transactionHistory.map(
                            (transaction, index) => (
                              <tr
                                key={index}
                              ...
                              >
                                ...
                                <td className="p-2 sm:p-3 text-right text-xs sm:text-sm">
                                  {formatCurrency(
                                    transaction.amounts.totalOrderValue
                                  )}
                                </td>
                                <td className="p-2 sm:p-3 text-right">
                                  {transaction.sellerPayout ? (
                                    <div>
                                      <span className="font-medium text-xs sm:text-sm">
                                        {formatCurrency(
                                          transaction.sellerPayout.netAmount
                                        )}
                                      </span>
                                      {transaction.dispute &&
                                        transaction.dispute.financialImpact >
                                          0 && (
                                ...
                                <td className="p-2 sm:p-3 text-right">
                                  {transaction.dispute &&
                                  transaction.dispute.financialImpact > 0 ? (
                                    <div>
              ...
              <div className="space-y-4 sm:">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <StatCard
                    title="Platform Revenue"
                    value={formatCurrency(
                      moneyFlowData.summary.totalPlatformRevenue
                    )}
                          ...
                          >
                            ...
                            <div className="flex justify-between items-start">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setDetailModal({ show: true, data: order })
                                }
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="bg-blue-50 rounded p-2">
                                ...
                                <p className="text-sm font-bold ">
                                  {formatCurrency(
                                    order.moneyFlow.buyerPayment.grandTotal
                                  )}
                                </p>
                              </div>
                              <div className="bg-green-50 rounded p-2">
                                ...
                                <p className="text-sm font-bold ">
                                  {formatCurrency(
                                    order.moneyFlow.platformRevenue
      ...
      {activeTab === "disputes" && dashboardData && (
        <div className="space-y-4 sm:">
          <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
            <h3 className="text-lg font-semibold mb-4">
              Dispute Financial Impact Analysis
            </h3>
                ...
                value={formatCurrency(
                  dashboardData.overview.disputes?.reduce((sum, d) => {
                    if (d._id === "BUYER") {
                      return (
                        sum +
                        (d.totalOrderValue * (d.avgRefundPercent || 0)) / 100
                      );
                    }
                    return sum;
                  }, 0) || 0
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="space-y-3">
                  {dashboardData.overview.disputes?.map((dispute, index) => {
                    ...
                    const percentage =
                      totalDisputes > 0
                        ? ((dispute.count / totalDisputes) * 100).toFixed(1)
                        : 0;
                      ...
                      <div key={index} className="space-y-1">
                        <div className="flex justify-between items-center">
                          ...
                          <span
                            className={`font-medium ${
                              dispute._id === "BUYER" ? "text-red-600" : ""
                            }`}
                          >
                        ...
                        {dispute.avgRefundPercent > 0 && (
                          <div className="text-xs text-gray-500">
                            Avg refund: {dispute.avgRefundPercent.toFixed(1)}%
                            of order value
                          </div>
                        )}
                      </div>
            ...
            {/* Dispute Trends */}
            {dashboardData.overview.disputes &&
              dashboardData.overview.disputes.length > 0 && (
              ...
              <div className="bg-white border rounded-lg p-4 mt-6">
                ...
                <div className="lg:hidden">
                  <div className="max-h-64 overflow-y-auto">
                    <div className="space-y-3">
                      {moneyFlowData.orders
                        ?.filter(
                          (order) => order.moneyFlow.disputeImpact.length > 0
                        )
                        .map((order, index) => (
                          <div
                            key={index}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="font-medium text-sm sm:text-base">
                    {detailModal.data.orderId}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Order Date</p>
                  <p className="font-medium text-sm sm:text-base">
                    {new Date(detailModal.data.orderDate).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Buyer</p>
                  <p className="font-medium text-sm sm:text-base">
                    {detailModal.data.buyer.name}
                  </p>
                  <p className="text-xs text-gray-500">
                    {detailModal.data.buyer.email}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Seller</p>
                  <p className="font-medium text-sm sm:text-base">
                    {detailModal.data.seller.name}
                  </p>
                  <p className="text-xs text-gray-500">
                    {detailModal.data.seller.email}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.buyerPayment.productAmount
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.buyerPayment.shippingCharge
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.buyerPayment
                            .buyerProtectionFee
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.buyerPayment.tax
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between font-bold text-base sm:text-lg col-span-1 sm:col-span-2 border-t pt-2">
                      ...
                      <span className="">
                        {formatCurrency(
                          detailModal.data.moneyFlow.buyerPayment.grandTotal
                        )}
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.platformRevenue
                            .buyerProtectionFee
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.platformRevenue.tax
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(
                          detailModal.data.moneyFlow.platformRevenue.taxCharges
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                  ...
                  {detailModal.data.moneyFlow.sellerPayouts.length > 0 ? (
                    <div className="space-y-3">
                      {detailModal.data.moneyFlow.sellerPayouts.map(
                        (payout, index) => (
                          <div
                            key={index}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                              ...
                              {payout.serviceCharge && (
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">
                                    {formatCurrency(
                                      parseFloat(payout.serviceCharge) || 0
                                    )}
                                  </span>
                                </div>
                              )}
                              {payout.withdrawfee && (
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">
                                    {formatCurrency(payout.withdrawfee || 0)}
                                  </span>
                                </div>
                              )}
                            </div>
                ...
                {detailModal.data.moneyFlow.disputeImpact.length > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      {detailModal.data.moneyFlow.disputeImpact.map(
                        (dispute, index) => (
                          <div
                            key={index}
                            className="bg-white p-3 rounded border"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                              ...
                              <div className="flex justify-between col-span-1 sm:col-span-2">
                                ...
                                <span className="font-medium text-red-600">
                                  {formatCurrency(dispute.financialImpact)}
                                </span>
                              </div>
                            </div>
                          </div>

...
import { getReportTypes, createReportType, updateReportType, deleteReportType, updateCancelType, createCancelType, deleteCancelType, getCancelTypes } from '../../features/slices/settingSlice';
...
const CancelTypeManagement  = () => {
  ...
  const fetchReportTypes = () => {
    dispatch(getCancelTypes({ pageNo: currentPage, size: pageSize }))
      .unwrap()
      .catch((error) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedReportType) {
        await dispatch(updateCancelType({ id: selectedReportType._id, ...formData })).unwrap();
        toast.success('Cancel type updated successfully');
      } else {
        await dispatch(createCancelType(formData)).unwrap();
        toast.success('Cancel type created successfully');
      }
      handleCloseModal();
      fetchReportTypes();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm('Are you sure you want to delete this report type?')) {
      try {
        ...
        toast.success('Cancel type deleted successfully');
        fetchReportTypes();
      } catch (error) {
...
export default CancelTypeManagement ; 